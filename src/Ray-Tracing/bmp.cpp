...
Color Bmp::GetSmoothColor( double u , double v ) {
	...
	int U1 = ( int ) floor( U + epsilon ) , U2 = U1 + 1;
	int V1 = ( int ) floor( V + epsilon ) , V2 = V1 + 1;
	double rat_U = U2 - U;
	double rat_V = V2 - V;
	if ( U1 < 0 ) U1 = strInfo.biHeight - 1; if ( U2 >= strInfo.biHeight ) U2 = 0;
	if (V1 < 0) V1 = strInfo.biWidth - 1; if (V2 >= strInfo.biWidth) V2 = 0;
	if (U2 < 0) U2 = strInfo.biHeight - 1; if (U1 >= strInfo.biHeight) U1 = 0;
	if (V2 < 0) V2 = strInfo.biWidth - 1; if (V1 >= strInfo.biWidth) V1 = 0;
	Color ret;
